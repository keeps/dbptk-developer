 ...
 */
@Test(groups = {"sqlserver-siard1"}) public class SqlServerTest {
        ...
        private Connection setup() throws SQLException {
                ...
                for (String dbname : Arrays.asList(db_source)) {
                        ...
                        exe.append("WHILE EXISTS(select NULL from sys.databases where name='").append(dbname)
                          .append("')\n");
                        exe.append("BEGIN\n");
                ...
                s.execute(
                  "CREATE TABLE dbo.Contacts ( phone_num UDT_phone NOT NULL, fax_num UDT_phone NULL) ON [PRIMARY]");
        ...
        @Test(description = "Sql server is available and accessible") public void testConnection()
          throws IOException, InterruptedException, SQLException {
                setup();

                Main.internal_main("-i", "SQLServerJDBC", "-is", SERVER_NAME, "-iin", INSTANCE_NAME, "-idb", db_source,
                  "-iu", USERNAME, "-ip", PASSWORD, "-e", "SIARD1", "-ef", SIARD_FILE, "-ide");
