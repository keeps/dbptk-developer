...
public class DB2Helper extends SQLHelper {
  ...
  public String getTriggersSQL(String schemaName, String tableName) {
    return "SELECT " + "trigname AS TRIGGER_NAME, trigtime AS ACTION_TIME, "
      + "trigevent AS TRIGGER_EVENT, text AS TRIGGERED_ACTION, " + "remarks AS REMARKS " + "FROM syscat.triggers "
      + "WHERE tabname='" + tableName + "' AND tabschema='" + schemaName + "'";
  }

  @Override
  public String getCheckConstraintsSQL(String schemaName, String tableName) {
    return "SELECT constname AS CHECK_NAME, text AS CHECK_CONDITION " + "FROM syscat.checks " + "WHERE tabschema='"
      + schemaName + "' AND tabname='" + tableName + "' AND type='C'";
  }
