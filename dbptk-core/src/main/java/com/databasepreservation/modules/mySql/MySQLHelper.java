...
public class MySQLHelper extends SQLHelper {
        ...
        @Override public String createTableSQL(TableStructure table) throws UnknownTypeException, ModuleException {
                return super.createTableSQL(table) + " ENGINE=INNODB";
        }

        @Override protected String createColumnSQL(ColumnStructure column, boolean isPrimaryKey, boolean isForeignKey)
          throws UnknownTypeException {
                return super.createColumnSQL(column, isPrimaryKey, isForeignKey) + (column.getDescription() != null ?
                  " COMMENT '" + escapeComment(column.getDescription()) + "'" :
                  "");
        }

        @Override protected String createTypeSQL(Type type, boolean isPkey, boolean isFkey)
          throws UnknownTypeException {
                String ret;
                ...
                if (MYSQL_TYPES.contains(type.getOriginalTypeName())) {
                        ...
                        if (!dateTime.getTimeDefined() && !dateTime.getTimeZoneDefined()) {
                                ...
                                if (dateTime.getTimeZoneDefined()) {
                                        logger
                                          .warn("Timezone not supported on MySQL: " + "defining type as 'datetime'");
                                }
                                ret = "datetime";
                        }
                        ...
                        if (length != null) {
                                if (type.getSql99TypeName().equalsIgnoreCase("BIT") || StringUtils
                                  .startsWithIgnoreCase(type.getSql99TypeName(), "BIT(")) {
                                        ...
                                        ret =
                                          "varbinary(" + (((length / 8.0) % 1 == 0) ? (length / 8) : ((length / 8) + 1))
                                            + ")";
                                } else {
                                        ret = "longblob";
                                }
         ...
         * @param fkey          the foreign key
         * @param addConstraint if an extra constraint info is need at creation of fkey
         * @param plus          a plus factor that makes a foreign key name unique
         * @return the SQL
        ...
        @Override public String getCheckConstraintsSQL(String schemaName, String tableName) {
                return "";
        }
        ...
        @Override public String getTriggersSQL(String schemaName, String tableName) {
                return "SELECT " + "trigger_name AS TRIGGER_NAME, " + "action_timing AS ACTION_TIME, "
                  ...
                  + "FROM information_schema.triggers " + "WHERE trigger_schema='" + schemaName + "' "
                  + "AND event_object_table='" + tableName + "'";
        }

        @Override public String getUsersSQL(String dbName) {
        ...
        @Override public String getDatabases(String database) {
                return "SHOW DATABASES LIKE '" + database + "\\_%';";
        }

        @Override public String dropDatabase(String database) {
                return "DROP DATABASE IF EXISTS " + database;
        }

        @Override protected String escapePrimaryKeyName(String pkey_name) {
