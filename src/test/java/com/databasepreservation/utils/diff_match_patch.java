 ...
 *
 * Modified to fit the needs of this project by Bruno Ferreira.
 *
 ...
 *
 * @author bferreira@keep.pt (Bruno Ferreira)
 */
...
public class diff_match_patch {
         ...
         */
        public float Diff_Timeout = 5.0f;
        /**
                                ...
                                if (overlap_length1 >= overlap_length2) {
                                        if (overlap_length1 >= deletion.length() / 2.0
                                          || overlap_length1 >= insertion.length() / 2.0) {
                                                ...
                                                prevDiff.text = deletion
                                                  .substring(0, deletion.length() - overlap_length1);
        ...
        public void diff_cleanupSemanticLossless(LinkedList<Diff> diffs) {
                ...
                while (nextDiff != null) {
                        if (prevDiff.operation == Operation.EQUAL && nextDiff.operation == Operation.EQUAL) {
                                ...
                                bestScore = diff_cleanupSemanticScore(equality1, edit) + diff_cleanupSemanticScore(edit,
                                  equality2);
                                while (edit.length() != 0 && equality2.length() != 0 && edit.charAt(0) == equality2
                                  .charAt(0)) {
                                        equality1 += edit.charAt(0);
                                        edit = edit.substring(1) + equality2.charAt(0);
                                        equality2 = equality2.substring(1);
                                        score =
                                          diff_cleanupSemanticScore(equality1, edit) + diff_cleanupSemanticScore(edit,
                                            equality2);
                                        ...
                                        if (count_delete + count_insert > 1) {
                                                ...
                                                if (both_types) {
                                                        ...
                                                        if (commonlength != 0) {
                                                                thisDiff = pointer.next();
                                                                thisDiff.text = text_insert
                                                                  .substring(text_insert.length() - commonlength)
                                                                  + thisDiff.text;
                                                                text_insert = text_insert
                                                                  .substring(0, text_insert.length() - commonlength);
                                                                text_delete = text_delete
                                                                  .substring(0, text_delete.length() - commonlength);
                                                                pointer.previous();
                                                        }
                                                }
     ...
     * Second pass: look for single edits surrounded on both sides by
         ...
         */
        public String diff_prettyHtml(LinkedList<Diff> diffs, String header) {
        ...
        /**
         * Convert a Diff list into a pretty CMD report.
         *
         * @param diffs LinkedList of Diff objects.
         * @return CMD representation.
         */
        public String diff_prettyCmd(LinkedList<Diff> diffs) {
                ...
                for (Diff aDiff : diffs) {
                        ...
                        switch (aDiff.operation) {
                                case INSERT:
                                        outputText.append(ANSI_BLACK).append(ANSI_BG_GREEN).append(text)
                                          .append(ANSI_RESET);
                                        break;
                                case DELETE:
                                        outputText.append(ANSI_BLACK).append(ANSI_BG_RED).append(text)
                                          .append(ANSI_RESET);
                                        break;
                                case EQUAL:
         ...
         * @return HTML representation.
         */
        public String diff_prettySimpleCmd(LinkedList<Diff> diffs) {
                String ANSI_RESET = "\u001B[0m";
                String ANSI_BLACK = "\u001B[30m";
                String ANSI_RED = "\u001B[31m";
                String ANSI_GREEN = "\u001B[32m";
                ...
                for (Diff aDiff : diffs) {
                        ...
                        switch (aDiff.operation) {
                                case INSERT:
                                        outputText.append(ANSI_GREEN).append("\n+++ ")
                                          .append(text.replace("\n", "\n+++ ")).append(ANSI_RESET);
                                        break;
                                case DELETE:
                                        outputText.append(ANSI_RED).append("\n--- ")
                                          .append(text.replace("\n", "\n--- ")).append(ANSI_RESET);
                                        break;
                                case EQUAL:
                                        outputText.append("\n    ").append(text.replace("\n", "\n    "));
