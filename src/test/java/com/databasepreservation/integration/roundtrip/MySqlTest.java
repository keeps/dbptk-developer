...
@Test(groups = {"mysql-siard1"}) public class MySqlTest {
        ...
        @BeforeClass public void setup() throws IOException, InterruptedException, URISyntaxException {
                ...
                Files.setAttribute(Paths.get(getClass().getResource("/mySql/scripts/setup.sh").getPath()),
                  "posix:permissions", executablePermissions);
                Files.setAttribute(Paths.get(getClass().getResource("/mySql/scripts/teardown.sh").getPath()),
                  "posix:permissions", executablePermissions);

                rt = new Roundtrip(String
                  .format("%s \"%s\" \"%s\" \"%s\" \"%s\"", getClass().getResource("/mySql/scripts/setup.sh").getPath(),
                    db_source, db_target, db_tmp_username, db_tmp_password), String
                  .format("%s \"%s\" \"%s\" \"%s\"", getClass().getResource("/mySql/scripts/teardown.sh").getPath(),
                    db_source, db_target, db_tmp_username), String
                  .format("mysql --user=\"%s\" --password=\"%s\" --database=\"%s\"", db_tmp_username, db_tmp_password,
                    db_source), String
                  ...
                  new String[] {"-i", "SIARD1", Roundtrip.TMP_FILE_SIARD_VAR, "-o", "MySQLJDBC", "localhost", db_target,
                    db_tmp_username, db_tmp_password});
        }
        ...
        @Test(description = "Tests MySQL files", dataProvider = "testFilesProvider", dependsOnMethods = {
