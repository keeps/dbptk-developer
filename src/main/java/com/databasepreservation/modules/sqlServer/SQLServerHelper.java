...
public class SQLServerHelper extends SQLHelper {
                        ...
                        if (string.isLengthVariable()) {
                                if (string.getLength().intValue() > 8000) {
                                        if (isPkey) {
                                                ret = "varchar(8000)";
                                                logger.warn(
                                                  "Resizing column length to 8000" + " so it can be a primary key");
                                        } else {
                                                ret = "text";
                                        }
                                } else {
        ...
        @Override protected String escapeTableName(String table) {
                return "[" + table + "]";
        }
        ...
        public String getTriggersSQL(String schemaName, String tableName) {
                return "SELECT o.name AS TRIGGER_NAME, " + "CAST(OBJECTPROPERTY(id, 'ExecIsAfterTrigger') AS char(1)) "
                  ...
                  + "INNER JOIN sys.schemas s ON tab.schema_id = s.schema_id " + "WHERE o.type = 'TR' AND tab.name = '"
