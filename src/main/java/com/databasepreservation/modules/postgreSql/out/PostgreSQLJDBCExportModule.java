...
public class PostgreSQLJDBCExportModule extends JDBCExportModule {
        ...
        public PostgreSQLJDBCExportModule(String hostname, int port, String database, String username, String password,
          boolean encrypt) {
                super("org.postgresql.Driver",
                  createConnectionURL(hostname, port, database, username, password, encrypt), new PostgreSQLHelper());
                this.hostname = hostname;
                this.port = port;
                this.database = database;
                this.username = username;
        ...
        public static String createConnectionURL(String hostname, int port, String database, String username,
          String password, boolean encrypt) {
                return "jdbc:postgresql://" + hostname + (port >= 0 ? ":" + port : "") + "/" + database + "?user="
                  + username + "&password=" + password + (encrypt ? "&ssl=true" : "");
        }
        ...
        @Override public void handleDataCloseTable(String schemaName, String tableId) throws ModuleException {
                try {
                        if (!currentIsIgnoredSchema) {
                                getStatement().executeUpdate(((PostgreSQLHelper) getSqlHelper())
                                  .grantPermissionsSQL(currentTableStructure.getId()));
                        }
                } catch (SQLException e) {
        ...
        @Override protected void handleSimpleTypeDateTimeDataCell(String data, PreparedStatement ps, int index,
          Cell cell, Type type) throws InvalidDateException, SQLException {
                ...
                if (dateTime.getTimeDefined()) {
                        if (type.getSql99TypeName().equalsIgnoreCase("TIME WITH TIME ZONE")) {
                                if (data != null) {
                                        ...
                                        logger.debug(
                                          "time with timezone after: " + time.toString() + "; timezone: " + cal
                                            .getTimeZone().getID());
        ...
        @Override protected void handleSimpleTypeNumericApproximateDataCell(String data, PreparedStatement ps,
          int index, Cell cell, Type type) throws NumberFormatException, SQLException {
                if (data != null) {
        ...
        @Override protected void handleSimpleTypeString(PreparedStatement ps, int index, BinaryCell bin)
