...
public class JDBCImportModule implements DatabaseImportModule {
         ...
         *
         * @param driverClassName the name of the the JDBC driver class
         * @param connectionURL   the connection url to use in the connection
         */
         ...
         * @throws SQLException
         * @throws ClassNotFoundException the JDBC driver could not be found in classpath
         */
         ...
         * @throws SQLException
         * @throws UnknownTypeException   the original data type is unknown
         * @throws ClassNotFoundException
         */
        ...
        /**
         * @param schema the schema structure
        ...
        protected List<TableStructure> getTables(SchemaStructure schema)
          throws SQLException, ClassNotFoundException, UnknownTypeException {
                ...
                ResultSet rset = getMetadata()
                  .getTables(dbStructure.getName(), schema.getName(), "%", new String[] {"TABLE"});
                int tableIndex = 1;
                while (rset.next()) {
         ...
         * @param nillable        is the column nillable
        ...
        protected ColumnStructure getColumnStructure(String tableName, String columnName, Type type, Boolean nillable,
          int index, String description, String defaultValue, Boolean isAutoIncrement) {
                ColumnStructure column = new ColumnStructure(tableName + "." + columnName, columnName, type, nillable,
                  description, defaultValue, isAutoIncrement);
                return column;
        }
                ...
                for (SchemaStructure schema : dbStructure.getSchemas()) {
                        for (TableStructure table : schema.getTables()) {
                                ResultSet rs = getMetadata()
                                  .getTablePrivileges(dbStructure.getName(), schema.getName(), table.getName());
                                while (rs.next()) {
                                        ...
                                        privilege
                                          .setObject("TABLE \"" + schema.getName() + "\".\"" + table.getName() + "\"");
         ...
         * @throws UnknownTypeException
         */
        protected List<ColumnStructure> getColumns(String schemaName, String tableName)
          throws SQLException, ClassNotFoundException, UnknownTypeException {
                ...
                while (rs.next()) {
                        ...
                        Type columnType = getType(schemaName, tableName, columnName, dataType, typeName, columnSize,
                          decimalDigits, numPrecRadix);

                        cLogMessage.append("Calculated type: ").append(columnType.getClass().getSimpleName())
                          .append("\n");
                        logger.trace(cLogMessage);

                        ColumnStructure column = getColumnStructure(tableName, columnName, columnType, isNullable,
                          index, remarks, defaultValue, isAutoIncrement);
         ...
         *
         * @param schemaName    The name of the associated schema, needed to resolve user
         *                      defined data types
         * @param tableName     The name of the associated table, needed to resolve user
         *                      defined data types
         * @param columnName    The name of the associated column, needed to resolve user
         *                      defined data types
         * @param dataType      the JDBC identifier of the original datatype
         * @param typeName      the name of the original data type
         * @param columnSize    the column size for the type
         * @param decimalDigits the number of decimal digits for the type
         * @param numPrecRadix  Indicates the numeric radix of this data type, which is usually 2 or 10
         * @return the normalized type
         * @throws UnknownTypeException   the original type is unknown and cannot be mapped
         * @throws ClassNotFoundException
         * @throws SQLException
         */
          ...
          throws UnknownTypeException, SQLException, ClassNotFoundException {
                ...
                switch (dataType) {
                        ...
                        case Types.NVARCHAR:
                                ...
                                break;
                        case Types.ARRAY:
                                Type subtype = getArraySubTypeFromTypeName(typeName, columnSize, numPrecRadix,
                                  numPrecRadix);
          ...
          throws UnknownTypeException {
                ...
                if (typeName.equals("_char")) {
                        ...
                        if (IGNORE_UNSUPPORTED_DATA_TYPES) {
                                return new UnsupportedDataType(Types.ARRAY, typeName, columnSize, decimalDigits,
                                  numPrecRadix);
                        } else {
         ...
         * @throws UnknownTypeException
         */
        protected Type getSpecificType(int dataType, String typeName, int columnSize, int decimalDigits,
         ...
         * @throws ModuleException
         */
        protected List<ForeignKey> getForeignKeys(String schemaName, String tableName)
          throws SQLException, UnknownTypeException, ClassNotFoundException {
          ...
          throws ClassNotFoundException {
                ...
                if (query != null) {
                        try {
                                ...
                                String message =
                                  "Check constraints were not imported for " + schemaName + "." + tableName + ". ";
                                if (query.equals("")) {
                        ...
                        logger.info(
                          "Check constraints were not imported: not supported yet on " + getClass().getSimpleName());
                }
                ...
                if (query != null) {
                        try {
                                ResultSet rs = getStatement()
                                  .executeQuery(sqlHelper.getTriggersSQL(schemaName, tableName));
        ...
        protected Row convertRawToRow(ResultSet rawData, TableStructure tableStructure)
          throws InvalidDataException, SQLException, ClassNotFoundException, ModuleException {
                Row row = null;
                if (isRowValid(rawData, tableStructure)) {
                        ...
                        for (int i = 0; i < tableStructure.getColumns().size(); i++) {
                                ...
                                cells.add(
                                  convertRawToCell(tableStructure.getName(), colStruct.getName(), i + 1, currentRow,
        ...
        protected Cell convertRawToCell(String tableName, String columnName, int columnIndex, long rowIndex,
          ...
          throws SQLException, InvalidDataException, ClassNotFoundException, ModuleException {
                ...
                if (cellType instanceof ComposedTypeArray) {
                        ...
                        logger.debug(
                          "Parsing array of subtype " + composedTypeArray.getElementType().getClass().getSimpleName());
        ...
        protected Cell rawToCellSimpleTypeNumericApproximate(String id, String columnName, Type cellType,
        ...
        protected Cell rawToCellSimpleTypeDateTime(String id, String columnName, Type cellType, ResultSet rawData)
          throws SQLException {
                ...
                if (undefinedDate.getTimeDefined()) {
                        if (cellType.getSql99TypeName().equalsIgnoreCase("TIME") || cellType.getSql99TypeName()
                          .equalsIgnoreCase("TIME WITH TIME ZONE")) {
        ...
        protected ResultSet getTableRawData(String tableId)
