...
public class SIARD1MetadataExportStrategy implements MetadataExportStrategy {
  ...
  public void writeMetadataXML(DatabaseStructure dbStructure, SIARDArchiveContainer container,
    ...
    try {
      xsdSchema = schemaFactory.newSchema(Paths.get(getClass().getResource(METADATA_XSD_RESOURCE_PATH).getPath())
        .toFile());
    } catch (SAXException e) {
      throw new ModuleException("XSD file has errors: " + getClass().getResource(METADATA_XSD_RESOURCE_PATH).getPath(),
        e);
    }
    ...
    try {
      ...
      throw new ModuleException("Metadata filename is invalid", e);
    }
  }
  ...
  public void writeMetadataXSD(DatabaseStructure dbStructure, SIARDArchiveContainer container,
    ...
    String xsdFilePath;
    try {
      xsdFilePath = metadataPathStrategy.getXsdFilePath(METADATA_FILENAME);
    } catch (InvalidParameterException e) {
      throw new ModuleException("Invalid metadata filename", e);
    }

    OutputStream out = writeStrategy.createOutputStream(container, xsdFilePath);
    ...
    try {
      ...
      throw new ModuleException("Could not write " + xsdFilePath + " in container " + container.toString(), e);
    }
