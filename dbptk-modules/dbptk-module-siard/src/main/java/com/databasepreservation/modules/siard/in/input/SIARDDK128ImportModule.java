 ...
 *
 */
public class SIARDDK128ImportModule {
  ...
  public SIARDDK128ImportModule(Path siardPackage, String paramImportAsSchema) {
    ...
    metadataStrategy = new SIARDDK128MetadataImportStrategy(pathStrategy, paramImportAsSchema);
    contentStrategy = new SIARDDK128ContentImportStrategy(readStrategy, pathStrategy, paramImportAsSchema);

  }

  public DatabaseImportModule getDatabaseImportModule() {
    final Map<String, String> properties = MapUtils.buildMapFromObjects(SIARDDK128ModuleFactory.PARAMETER_FOLDER,
      mainContainer.getPath().normalize().toAbsolutePath().toString(), SIARDDK128ModuleFactory.PARAMETER_AS_SCHEMA,
      paramImportAsSchema);
