 ...
 *
 */
public class SIARDDK1007MetadataExportStrategy implements MetadataExportStrategy {
  ...
  public SIARDDK1007MetadataExportStrategy(SIARDDK1007ExportModule siarddk1007ExportModule) {
    siardMarshaller = siarddk1007ExportModule.getSiardMarshaller();
    ...
    metadataPathStrategy = siarddk1007ExportModule.getMetadataPathStrategy();
    exportModuleArgs = siarddk1007ExportModule.getExportModuleArgs();
  ...
  public void writeMetadataXML(DatabaseStructure dbStructure, SIARDArchiveContainer outputContainer,
    ...
    try {
      IndexFileStrategy tableIndexFileStrategy = new SIARDDK1007TableIndexFileStrategy(lobsTracker);
      String path = metadataPathStrategy.getXmlFilePath(SIARDDKConstants.TABLE_INDEX);
      OutputStream writer = SIARDDK1007FileIndexFileStrategy.getWriter(outputContainer, path, writeStrategy);
      ...
      SIARDDK1007FileIndexFileStrategy.addFile(path);
    ...
    if (lobsTracker.getLOBsCount() > 0) {
      try {
        ...
        siardMarshaller.marshal(SIARDDKConstants.JAXB_CONTEXT_DOCINDEX,
          ...
          SIARDDK1007DocIndexFileStrategy.generateXML(dbStructure));
  ...
  private void writeSchemaFile(SIARDArchiveContainer container, String indexFile, WriteStrategy writeStrategy)
    ...
    OutputStream outputStream = SIARDDK1007FileIndexFileStrategy.getWriter(container, path, writeStrategy);
