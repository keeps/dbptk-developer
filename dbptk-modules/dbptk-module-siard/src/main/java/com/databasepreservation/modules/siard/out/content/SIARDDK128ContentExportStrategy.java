 ...
 *
 */
public class SIARDDK128ContentExportStrategy implements ContentExportStrategy {
  ...
  private static final Logger logger = LoggerFactory.getLogger(SIARDDK128ContentExportStrategy.class);
  ...
  private final SIARDDK128FileIndexFileStrategy SIARDDK128FileIndexFileStrategy;
  private final SIARDDK128DocIndexFileStrategy SIARDDK128DocIndexFileStrategy;
  ...
  public SIARDDK128ContentExportStrategy(SIARDDK128ExportModule siarddk128ExportModule) {
    ...
    contentPathExportStrategy = siarddk128ExportModule.getContentPathExportStrategy();
    ...
    baseContainer = siarddk128ExportModule.getMainContainer();
    writeStrategy = siarddk128ExportModule.getWriteStrategy();
  ...
  public void openTable(TableStructure tableStructure) throws ModuleException {
    ...
    tableXmlOutputStream = SIARDDK128FileIndexFileStrategy.getWriter(baseContainer,
    ...
    tableXsdOutputStream = SIARDDK128FileIndexFileStrategy.getLOBWriter(baseContainer,
    ...
    try {
      ...
      SIARDDK128FileIndexFileStrategy.addFile(contentPathExportStrategy.getTableXsdFilePath(0, tableCounter));
  ...
  public void closeTable(TableStructure tableStructure) throws ModuleException {
    try {
      ...
      SIARDDK128FileIndexFileStrategy.addFile(contentPathExportStrategy.getTableXmlFilePath(0, tableCounter));
      ...
      for (Cell cell : row.getCells()) {
        ...
        if (lobsTracker.getLOBsType(tableCounter, columnIndex) == null) {
          ...
          if (cell instanceof NullCell) {
            ...
            // Write the BLOB
            OutputStream out = SIARDDK128FileIndexFileStrategy.getLOBWriter(baseContainer, blob.getOutputPath(), writeStrategy);
            ...
            SIARDDK128DocIndexFileStrategy.addDoc(lobsTracker.getLOBsCount(), 0, 1, lobsTracker.getDocCollectionCount(),
              "originalFilename", fileExtension, null);

            // Add file to fileIndex
            SIARDDK128FileIndexFileStrategy.addFile(blob.getOutputPath());
