 ...
 *
 */
public class SIARDDK2020ContentExportStrategy implements ContentExportStrategy {
  ...
  private static final Logger logger = LoggerFactory.getLogger(SIARDDK2020ContentExportStrategy.class);
  ...
  private final SIARDDK2020FileIndexFileStrategy SIARDDK2020FileIndexFileStrategy;
  private final SIARDDK2020DocIndexFileStrategy SIARDDK2020DocIndexFileStrategy;
  ...
  public SIARDDK2020ContentExportStrategy(SIARDDK2020ExportModule siarddk2020ExportModule) {
    ...
    contentPathExportStrategy = siarddk2020ExportModule.getContentPathExportStrategy();
    SIARDDK2020FileIndexFileStrategy = siarddk2020ExportModule.getFileIndexFileStrategy();
    SIARDDK2020DocIndexFileStrategy = siarddk2020ExportModule.getDocIndexFileStrategy();
    baseContainer = siarddk2020ExportModule.getMainContainer();
    writeStrategy = siarddk2020ExportModule.getWriteStrategy();
    lobsTracker = siarddk2020ExportModule.getLobsTracker();
  }
  ...
  public void openTable(TableStructure tableStructure) throws ModuleException {
    ...
    tableXmlOutputStream = SIARDDK2020FileIndexFileStrategy.getWriter(baseContainer,
    ...
    tableXsdOutputStream = SIARDDK2020FileIndexFileStrategy.getLOBWriter(baseContainer,
    ...
    try {
      ...
      SIARDDK2020FileIndexFileStrategy.addFile(contentPathExportStrategy.getTableXsdFilePath(0, tableCounter));
  ...
  public void closeTable(TableStructure tableStructure) throws ModuleException {
    try {
      ...
      SIARDDK2020FileIndexFileStrategy.addFile(contentPathExportStrategy.getTableXmlFilePath(0, tableCounter));
      ...
      for (Cell cell : row.getCells()) {
        ...
        if (lobsTracker.getLOBsType(tableCounter, columnIndex) == null) {
          ...
          if (cell instanceof NullCell) {
            ...
            // Write the BLOB
            OutputStream out = SIARDDK2020FileIndexFileStrategy.getLOBWriter(baseContainer, blob.getOutputPath(), writeStrategy);
            ...
            SIARDDK2020DocIndexFileStrategy.addDoc(lobsTracker.getLOBsCount(), 0, 1, lobsTracker.getDocCollectionCount(),
              "originalFilename", fileExtension, null);

            // Add file to fileIndex
            SIARDDK2020FileIndexFileStrategy.addFile(blob.getOutputPath());
