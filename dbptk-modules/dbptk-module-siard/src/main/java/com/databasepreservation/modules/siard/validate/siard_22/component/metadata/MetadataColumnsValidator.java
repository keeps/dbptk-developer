 ...
 */
package com.databasepreservation.modules.siard.validate.siard_22.component.metadata;
...
public class MetadataColumnsValidator extends MetadataValidator {
  ...
  public boolean validate() throws ModuleException {
    ...
    String archiveLobFolder;
    try (
      InputStream isArchive = zipFileManagerStrategy.getZipInputStream(path,
    ...
    if (this.skipAdditionalChecks) {
      ...
      if (validateColumnLobFolders(tables, archiveLobFolder)) {
   ...
   * A_M_5.6-1-2
   */
  private boolean validateColumnLobFolders(NodeList nodes, String archiveLobFolder) {
    boolean hasErrors = false;
  ...
  private boolean validateLobFolder(String schemaFolder, String tableFolder, String type, String folder, String column,
    String path, String archiveLobFolder) {
    boolean hasErrors = false;
            ...
            if (streamReader.getAttributeCount() > 0) {
              ...
              if (!fileName.isEmpty()) {
                if (Paths.get(fileName).getName(0).toString().equals(Constants.SIARD_CONTENT_FOLDER)) {
                  ...
                  if (!lobFolderFile.exists() && !lobFolderPartFile.exists()) {
