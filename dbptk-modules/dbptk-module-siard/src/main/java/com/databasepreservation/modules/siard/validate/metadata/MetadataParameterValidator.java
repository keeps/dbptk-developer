...
public class MetadataParameterValidator extends MetadataValidator {
  ...
  public boolean validate() {
    ...
    return reportValidations(M_516_1, PARAMETER) && reportValidations(M_516_1_1, PARAMETER_NAME)
      && reportValidations(M_516_1_2, PARAMETER_MODE) && reportValidations(M_516_1_8, PARAMETER_DESCRIPTION);
  }
  ...
  private boolean readXMLMetadataParameterLevel() {
    try (ZipFile zipFile = new ZipFile(getSIARDPackagePath().toFile())) {
      ...
      for (int i = 0; i < nodes.getLength(); i++) {
        ...
        String schema = MetadataXMLUtils.getParentNameByTagName(view, SCHEMA);

        String name = MetadataXMLUtils.getChildTextContext(view, PARAMETER_NAME);
        ...
        String description = MetadataXMLUtils.getChildTextContext(view, PARAMETER_DESCRIPTION);
  ...
  private boolean validateParameterName(String name, String schema) {
    ...
    if (name == null || name.isEmpty()) {
      addWarning(PARAMETER_NAME, String.format("Parameter name should exist inside schema %s", schema));
    }
    // M_5.16-1-1
    if (!checkDuplicates.add(name)) {
      addWarning(PARAMETER_NAME, String.format("Parameter name %s inside schema %s should be unique", name, schema));
    }
  ...
  private boolean validateParameterDescription(String description, String schema, String name) {
    return validateXMLField(description, PARAMETER_DESCRIPTION, false, true, SCHEMA, schema, PARAMETER_NAME, name);
  }
}
